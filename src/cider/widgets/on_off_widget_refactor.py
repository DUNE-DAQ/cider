...
class OnOffGridWidget(DaqWidget):
    def __init__(
        ...
        self.switches = {
            label: self.generate_switch(label, enabled)
            for label, enabled in switch_labels.items()
        }
    ...
    def set_switch_action(self, button_label: str, action: Callable):
        if button_label in self.switches:
            self.switches[button_label][1].add_action_sequence(
                "switch_changed", [action]
            )
    ...
    def on_switch_changed(self, event: Switch.Changed):
        ...
        switch_to_query.do_action_sequence("switch_changed", disable=disable_value)
...
class DisableComponentWidget(OnOffGridWidget):
    def __init__(
        ...
        disabled_objs = GetAttributeAction(configuration)(
            GetDalObjectAction(self._configuration)(session_name, "Session"), "disabled"
        )

        label_dict = {
            GetAttributeAction(configuration)(obj, "id"): obj in disabled_objs
        ...
        for label, dal in zip(label_dict.keys(), disabled_objs):
            ...
            partial_action = partial(
                full_action_class, dal=dal, session_name=session_name
            )
