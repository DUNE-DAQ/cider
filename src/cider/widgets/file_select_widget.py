...
class FileSelectWidget(Static):
    def __init__(self, session_directories: str | List[str] = "", renderable = "", *, expand: bool = False, shrink: bool = False, markup: bool = True, name: str | None = None, id: str | None = None, classes: str | None = None, disabled: bool = False) -> None:
        super().__init__(renderable, expand=expand, shrink=shrink, markup=markup, name=name, id=id, classes=classes, disabled=disabled)

        self._database_list = self.generate_selection_list(session_directories)
    ...
    def get_number_of_sessions(cls, config_file_path: str)->int:
        ...
        try:
            ...
            n_sessions = len(GetDalsOfClassAction(config_file)("Session"))        
        except:
            n_sessions = 0
